...
# set up the grid
param_grid = { 'C': Cs.get(), 'kernel': ['linear'] }
...
# -------------------------------------------------------------------------------
def setdiff1d_cupy(arr1, arr2):
    # Sort both arrays
    arr1_sorted = np.sort(arr1)
    arr2_sorted = np.sort(arr2)
    
    # Use the cupy.in1d function to check which elements of arr1 are not in arr2
    mask = ~np.in1d(arr1_sorted, arr2_sorted)
    
    # Return the elements that are in arr1 but not in arr2
    return arr1_sorted[mask]
...
def fnc_fit_and_score(data_slice, tri_ind, hold_out, n_cvs, afc, labs, label, thresh, grid, seeds):
    ...
    for i in range(n_cvs):
        ...
        trn_ind = np.random.choice(setdiff1d_cupy( tri_ind, tst_ind ), size = len(setdiff1d_cupy( tri_ind, tst_ind )), replace = True)

        # get the training data (X) and the training labels (y)
        X = data_slice[trn_ind,:]
        y = np.select([labs[trn_ind] >= thresh[1], labs[trn_ind] <= thresh[0]], [np.array(0), np.array(1)], default=0)
        
        # fit the model
        grid.fit( np.asnumpy(X), np.asnumpy(y) )

        # get the test data (X) and the test labels (y)
        X_test = data_slice[tst_ind, :]
        y_test = np.select([labs[tst_ind] >= thresh[1], labs[tst_ind] <= thresh[0]], [np.array(0), np.array(1)], default=0)

        # predict and score
        y_pred = grid.predict(np.asnumpy(X_test))
        cm[i] = confusion_matrix(np.asnumpy(y_test), np.asnumpy(y_pred), normalize = "true").diagonal()
