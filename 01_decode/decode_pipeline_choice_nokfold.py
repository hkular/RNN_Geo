...
# Description: decoding RNN firing rate and trial choice
...
# not doing k-fold cross validation with neutral dataset because expect >100 samples per class in a 70-30 split of train and test
...
# define object - use a SVC that balances class weights 
...
def fnc_decode_times(n_boot):
    ...
    for t in times:
        ...
        result = fnc_fit_and_score(np.mean(data_d[:, t, :], axis=1), tri_ind, hold_out, labs, thresh, grid, seed)
...
def fnc_fit_and_score(data_slice, tri_ind, hold_out, labs, thresh, grid, seeds):
    """
    ...
    # trials to hold out as test set
    tst_ind = np.random.choice(tri_ind, size = hold_out, replace = False)
    ...
    cm = confusion_matrix(y_test, y_pred, normalize = "true").diagonal()
...
modelnum = 2 # which model do we want ranges 0 to 2?
...
for afc, coh, fr in combinations:
    ...
    data_file = f"{data_dir}/Trials{task_info['trials']}_model{model_path[-7:-4]}_neutral.npz"
    data = np.load(data_file)
    data_d = data[f'fr{fr}'] # this is a [trial x time step x unit] matrix
    labs = data['outs'][:,-1] # [trial x time step]

    # get some info about structure of the data
    tris = data_d.shape[0]             # number of trials
    tri_ind = np.arange(0,tris)      # list from 0...tris
    hold_out = int( (tris) * .3 )   # how many trials to hold out 70-30 split
    ...
    full_file = f"/mnt/neurocube/local/serenceslab/holly/RNN_Geo/data/decoding/{D_params['pred']}/fr{fr}/{coh}_{afc}afc/boot_{D_params['pred']}_choice{modelnum}.npz"
